...
项目面向三轮全向底盘，使用 STM32F103、Keil MDK、标准外设库和 FreeRTOS，实现了从 ROS 速度指令到电机闭环控制，再到底盘速度反馈的完整链路。
...
> 注意：当前 Keil 工程文件 `STM32 工程模版.uvprojx` 中配置的目标器件是 `STM32F103RB`。如果你的实际开发板使用 `STM32F103RCT6` 或其他型号，请在 Keil 中核对芯片型号、Flash 容量和启动文件。
